        ...
        for line in f:
            ...
            if len(parts) >= 11:
                try:
                    ...
                    # Compute N_rot variants = tau / tau_rot
                    row = {
                        'Pe': float(parts[1]),
                        'T': float(parts[2]),
                        'kappa': float(parts[3]),
                        'N_rot_eff': tau_eff / tau_rot if tau_rot > 0 else np.nan,
                        'N_rot_star': tau_star / tau_rot if tau_rot > 0 else np.nan,
                        'N_rot_geom': tau_geom / tau_rot if tau_rot > 0 else np.nan,
                        'N_rot_0': tau_0 / tau_rot if tau_rot > 0 else np.nan,
                        'tau_rot': tau_rot,
                    }
                    data.append(row)
...
#
# Rotational number data (N_rot = τ_trans / τ_rot):
